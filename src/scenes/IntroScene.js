...
export class IntroScene extends Phaser.Scene {
  ...
  async create() {
    ...
    previews.forEach((p, i) => { if (i !== style) { p.destroy(); } });
    sel.destroy();
    const me = previews[style];
  ...
  pickStyle(previews, sel) {
    ...
    return new Promise((resolve) => {
      const tick = () => {
        if (input.nav('left') ) { i = (i + 3) % 4; audio.sfx('cursor'); draw(); }
        else if (input.nav('right')) { i = (i + 1) % 4; audio.sfx('cursor'); draw(); }
